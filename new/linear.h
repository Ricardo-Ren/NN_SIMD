...
struct Linear {
  ...
  */
  ...
  void forward_cpu_simd(tensor<real,M,K0,K1,K2>& x, int training) {
    ...
    y.set_n0(m);
    x_ptr = &x;
    constexpr int VLEN = (N < 16) ? N : 16;
    __m512 in_vec, out_vec, weight_vec;
    float y_arr[16], weight_arr[16];
    for (idx_t i = 0; i < m; i++) {
      for (idx_t j = 0; j < N; j += VLEN) {
        out_vec = _mm512_set1_ps(0);
        // real v = 0.0;
        for (idx_t k0 = 0; k0 < K0; k0++) {
          for (idx_t k1 = 0; k1 < K1; k1++) {
            for (idx_t k2 = 0; k2 < K2; k2++) {
              in_vec = _mm512_set1_ps(x(i,k0,k1,k2));
              for (idx_t idx = 0; idx < VLEN; idx++) {
                weight_arr[idx] = w(k0,k1,k2,j+idx);
              }
              weight_vec = _mm512_load_ps(&weight_arr[0]);
              out_vec = _mm512_fmadd_ps(in_vec, weight_vec, out_vec);
              // v += x(i,k0,k1,k2) * w(k0,k1,k2,j);
            }
          }
        }
        _mm512_store_ps(&y_arr[0], out_vec);
        for (idx_t idx = 0; idx < VLEN; idx++) {
          y(i,j+idx) = y_arr[idx] + b(j+idx);
        }
        // y(i,j) = v + b(j);
      }
    }
  ...
  */
  ...
  void backward_cpu_simd(tensor<real,M,N>& gy) {
    ...
    gb.set_n0(N);
    gx.set_n0(m);
    const int N_VLEN = (N < 16) ? N : 16;     // use _mm512_* intrisics
    __m512 in_vec, out_vec, weight_vec, gy_vec; 
    float gy_arr[16], gw_arr[16], gx_arr[16], gb_arr[16];
    tensor<real,M,K0,K1,K2>& x = *x_ptr;
    for (idx_t k0 = 0; k0 < K0; k0++) {
      for (idx_t k1 = 0; k1 < K1; k1++) {
        for (idx_t k2 = 0; k2 < K2; k2++) {
          for (idx_t j = 0; j < N; j += N_VLEN) {
            out_vec = _mm512_set1_ps(0);
            // real v = 0.0;
            for (idx_t i = 0; i < m; i++) {
              in_vec = _mm512_set1_ps(x(i,k0,k1,k2));
              for (idx_t idx = 0; idx < N_VLEN; idx++) {
                gy_arr[idx] = gy(i,j+idx);
              }
              gy_vec = _mm512_load_ps(&gy_arr[0]);
              out_vec = _mm512_fmadd_ps(gy_vec, in_vec, out_vec);
              // v += gy(i,j) * x(i,k0,k1,k2);
            }
            _mm512_store_ps(&gw_arr[0], out_vec);
            for (idx_t idx = 0; idx < N_VLEN; idx++) {
              gw(k0,k1,k2,j+idx) = gw_arr[idx];
            }
            // gw(k0,k1,k2,j) = v;
          }
        }
    ...
    for (idx_t j = 0; j < N; j += N_VLEN) {
      ...
      _mm512_store_ps(&gb_arr[0], out_vec);
      for (idx_t idx = 0; idx < N_VLEN; idx++) {
        gb(j+idx) = gb_arr[idx];
      }
      // gb(j) = v;
    ...
    const int i_VLEN = (m < 16) ? m : 16;     // use _mm512_* intrisics
    for (idx_t i = 0; i < m; i += i_VLEN) {
      for (idx_t k0 = 0; k0 < K0; k0++) {
        for (idx_t k1 = 0; k1 < K1; k1++) {
          for (idx_t k2 = 0; k2 < K2; k2++) {
            out_vec = _mm512_set1_ps(0);
            // real v = 0.0;
            for (idx_t j = 0; j < N; j++) {
              weight_vec = _mm512_set1_ps(w(k0,k1,k2,j));
              for (idx_t idx = 0; idx < i_VLEN; idx++) {
                gy_arr[idx] = gy(i+idx,j);
              }
              gy_vec = _mm512_load_ps(&gy_arr[0]);
