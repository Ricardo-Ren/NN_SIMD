...
struct Convolution2D {
  ...
  */
  ...
  void forward_cpu_simd(tensor<real,maxB,IC,H,W>& x, int training) {
    (void)training;
    constexpr int VLEN = (OC < 16) ? OC : 16;     // use _mm512_* intrisics
    idx_t B = x.n0;             // batch size
    y.set_n0(B);
    x_ptr = &x;                 // save pointer to input for backward
    __m512 in_vec, out_vec, weight_vec; 
    float weights[16], outs[16];
    for (idx_t s = 0; s < B; s++) {       // for each sample
      for (idx_t oc = 0; oc < OC; oc += VLEN) { // for each output channel
        for (idx_t i = 0; i < H - K + 1; i++) {    // for each output pixel
          for (idx_t j = 0; j < W - K + 1; j++) { // for each output pixel
            // calculate a single output pixel
            out_vec = _mm512_set1_ps(0);
            // real v = 0.0;
            for (idx_t ic = 0; ic < IC; ic++) { // input channel
              for (idx_t di = 0; di < K; di++) {
                for (idx_t dj = 0; dj < K; dj++) {
                  in_vec = _mm512_set1_ps(x(s,ic,i+di,j+dj));
                  for (idx_t idx = 0; idx < VLEN; idx++) {
                    weights[idx] = w(oc+idx,ic,di,dj);
                  }
                  weight_vec = _mm512_load_ps(&weights[0]);
                  out_vec = _mm512_fmadd_ps(weight_vec, in_vec, out_vec);
                  // v += w(oc,ic,di,dj) * x(s,ic,i+di,j+dj);
                }
              }
            }
            _mm512_store_ps(&outs[0], out_vec);
            for (idx_t idx = 0; idx < VLEN; idx++) {
              y(s,oc+idx,i,j) = outs[idx] + b(oc+idx);
            }
            // y(s,oc,i,j) = v + b(oc);
          }
        }
...
float sum(__m256 x) {
 ...
 __device__ __host__ 
  void forward_simd_ilp(tensor<real,maxB,IC,H,W>& x, int training) {
    (void)training;
    constexpr int VLEN = 8;     // to use _mm256_* intrisics
    idx_t B = x.n0;             // batch size
    y.set_n0(B);
    x_ptr = &x;                 // save pointer to input for backward
    __m256 in_vec, weight_vec; // simd vectors
    for (idx_t s = 0; s < B; s++) {       // for each sample
      for (idx_t oc = 0; oc < OC; oc += VLEN) { // for each output channel
        for (idx_t i = 0; i < H - K + 1; i++) {   // for each output pixel
          for (idx_t j = 0; j < W - K + 1; j++) { // for each output pixel
            // calculate a single output pixel
            __m256 out_vec[VLEN];
            for (idx_t dk = 0; dk < VLEN; dk++) {
              out_vec[dk] = _mm256_set1_ps(0);
            }

            //real v = 0.0;
            for (idx_t ic = 0; ic < IC; ic += VLEN) { // input channel
              for (idx_t di = 0; di < K; di++) {
                for (idx_t dj = 0; dj < K; dj++) {
                  //v += w(oc,ic,di,dj) * x(s,ic,i+di,j+dj);
                  in_vec = _mm256_set_ps(x(s,ic+7,i+di,j+dj),
                                          x(s,ic+6,i+di,j+dj),
                                          x(s,ic+5,i+di,j+dj),
                                          x(s,ic+4,i+di,j+dj),
                                          x(s,ic+3,i+di,j+dj),
                  ...
                  for (idx_t dk = 0; dk < VLEN; dk++) {
                    weight_vec = _mm256_set_ps(w(oc+dk,ic+7,di,dj),
                                               w(oc+dk,ic+6,di,dj),
                                               w(oc+dk,ic+5,di,dj),
                                               w(oc+dk,ic+4,di,dj),
                                               w(oc+dk,ic+3,di,dj),
                    ...
                    out_vec[dk] = _mm256_fmadd_ps(weight_vec, in_vec, out_vec[dk]);
                    // out_vec[dk] = _mm256_add_ps(out_vec[dk], bias_vec);
                  }
                }
              }
            }
            ...
            // y(s,oc,i,j) = v + b(oc);
            for (idx_t dk = 0; dk < VLEN; dk++) {
              float val = sum(out_vec[dk]);
              y(s,oc+dk,i,j) = (real)val + b(oc+dk);
            }
          }
        }
      }
  ...
  */
  ...
  void backward_cpu_simd(tensor<real,maxB,OC,H-K+1,W-K+1>& gy) {
    idx_t B = gy.n0;
    constexpr int OC_VLEN = (OC < 16) ? OC : 16;     // use _mm512_* intrisics
    gw.set_n0(OC);
    gb.set_n0(OC);
    gx.set_n0(B);
    tensor<real,maxB,IC,H,W>& x = *x_ptr;
    __m512 in_vec, out_vec, gy_vec, weight_vec; 
    float gy_arr[16], v_arr[16];
    for (idx_t oc = 0; oc < OC; oc += OC_VLEN) {   // output channel
      for (idx_t ic = 0; ic < IC; ic++) { // input channel
        for (idx_t di = 0; di < K; di++) { // kernel pixel
          for (idx_t dj = 0; dj < K; dj++) { // kernel pixel
            out_vec = _mm512_set1_ps(0);
            ...
            for (idx_t s = 0; s < B; s++) { // training samples
              for (idx_t i = 0; i < H - K + 1; i++) { // sample pixel
                for (idx_t j = 0; j < W - K + 1; j++) { // sample pixel
                  in_vec = _mm512_set1_ps(x(s,ic,i+di,j+dj));
                  for (idx_t idx = 0; idx < OC_VLEN; idx++) {
                    gy_arr[idx] = gy(s,oc+idx,i,j);
                  }
                  gy_vec = _mm512_load_ps(&gy_arr[0]);
                  out_vec = _mm512_fmadd_ps(gy_vec, in_vec, out_vec);
                  // v += gy(s,oc,i,j) * x(s,ic,i+di,j+dj);
                }
              }
            }
            _mm512_store_ps(&v_arr[0], out_vec);
            for (idx_t idx = 0; idx < OC_VLEN; idx++) {
              gw(oc+idx,ic,di,dj) = v_arr[idx];
            }
            // gw(oc,ic,di,dj) = v;
          }
        }
    ...
    for (idx_t oc = 0; oc < OC; oc += OC_VLEN) {
      out_vec = _mm512_set1_ps(0);
      // real v = 0.0;
      for (idx_t s = 0; s < B; s++) {
        for (idx_t i = 0; i < H - K + 1; i++) {
          for (idx_t j = 0; j < W - K + 1; j++) {
            ...
            gy_vec = _mm512_load_ps(&gy_arr[0]);
            out_vec = _mm512_add_ps(gy_vec, out_vec);
            // v += gy(s,oc,i,j);
          }
        }
      }
    ...
    constexpr int IC_VLEN = (IC < 16) ? IC : 16; 
    float weight_arr[IC_VLEN], gx_arr[16];
    for (idx_t s = 0; s < B; s++) {
      for (idx_t ic = 0; ic < IC; ic += IC_VLEN) {
        for (idx_t i = 0; i < H; i++) {
          for (idx_t j = 0; j < W; j++) {
            out_vec = _mm512_set1_ps(0);
            // real v = 0.0;
            for (idx_t oc = 0; oc < OC; oc++) {
              for (idx_t di = 0; di < K; di++) {
                for (idx_t dj = 0; dj < K; dj++) {
                  if (0 <= i - di && i - di < H - K + 1
                      && 0 <= j - dj && j - dj < W - K + 1) {
                    gy_vec = _mm512_set1_ps(gy(s,oc,i-di,j-dj));
                    for (idx_t idx = 0; idx < IC_VLEN; idx++) {
                      weight_arr[idx] = w(oc,ic+idx,di,dj);
                    }
                    weight_vec = _mm512_load_ps(&weight_arr[0]);
                    out_vec = _mm512_fmadd_ps(gy_vec, weight_vec, out_vec);
                    // v += gy(s,oc,i-di,j-dj) * w(oc,ic,di,dj);
                  }
                }
              }
            }
            _mm512_store_ps(&gx_arr[0], out_vec);
            for (idx_t idx = 0; idx < IC_VLEN; idx++) {
              gx(s,ic+idx,i,j) = gx_arr[idx];
            }
            // gx(s,ic,i,j) = v;
          }
        }
