...
struct MaxPooling2D {
  ...
  */
  ...
  __device__ __host__
  void forward_cpu_simd(tensor<real,maxB,C,H,W>& x, int training) {
    (void)training;
    constexpr int VLEN = (C < 16) ? C : 16;
    const idx_t B = x.n0;
    y.set_n0(B);
    argmax_i.set_n0(B);
    argmax_j.set_n0(B);

    __m512 x_vec, v_vec, max_i_vec, max_j_vec;
    __mmask16 max_mask;
    float x_arr[16], v_arr[16], i_arr[16], j_arr[16];

    for (idx_t s = 0; s < B; s++) {
      for (idx_t c = 0; c < C; c += VLEN) {
        for (idx_t i = 0; i < H/S; i++) {
          for (idx_t j = 0; j < W/S; j++) {
            max_i_vec = _mm512_set1_ps(S * i);
            max_j_vec = _mm512_set1_ps(S * j);
            for (idx_t idx = 0; idx < VLEN; idx++) {
              v_arr[idx] = x(s,c+idx,S*i,S*j);
            }
            v_vec = _mm512_load_ps(&v_arr[0]);
            // real v = x(s,c,max_i,max_j);
            for (idx_t i_ = S * i; i_ < S * (i + 1); i_++) {
              for (idx_t j_ = S * j; j_ < S * (j + 1); j_++) {
                for (idx_t idx = 0; idx < VLEN; idx++) {
                  x_arr[idx] = x(s,c+idx,i_,j_);
                  i_arr[idx] = i_;
                  j_arr[idx] = j_;
                }
                x_vec = _mm512_load_ps(&x_arr[0]);
                max_mask = _mm512_cmp_ps_mask(v_vec, x_vec, _CMP_LT_OS);
                max_i_vec = _mm512_mask_load_ps(max_i_vec, max_mask, &i_arr[0]);
                max_j_vec = _mm512_mask_load_ps(max_j_vec, max_mask, &j_arr[0]);
                v_vec = _mm512_mask_load_ps(v_vec, max_mask, &x_arr[0]);
                // if (v < x(s,c,i_,j_)) {
                //   max_i = i_;
                //   max_j = j_;
                //   v = x(s,c,max_i,max_j);
            ...
            _mm512_store_ps(&v_arr[0], v_vec);
            for (idx_t idx = 0; idx < VLEN; idx++) {
              y(s,c+idx,i,j) = v_arr[idx];
            }
            // y(s,c,i,j) = v;
            _mm512_store_ps(&i_arr[0], max_i_vec);
            for (idx_t idx = 0; idx < VLEN; idx++) {
              argmax_i(s,c+idx,i,j) = i_arr[idx];
            }
            // argmax_i(s,c,i,j) = max_i;
            _mm512_store_ps(&j_arr[0], max_j_vec);
            for (idx_t idx = 0; idx < VLEN; idx++) {
              argmax_j(s,c+idx,i,j) = j_arr[idx];
            }
            // argmax_j(s,c,i,j) = max_j;
          }
        }
